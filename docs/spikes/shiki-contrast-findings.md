...
- **Comment color** `#6A737D` (ratio 3.05 : 1) → overridden to `#8b949e` (ratio 5.07 : 1) via `!important` CSS rule in `styles.css` lines 52–56. This fix was correct and sufficient.
...
**Recommendation: Option D** — add `!important` to `pre { background }` so the CSS light background wins in light mode, and then address the 11 token failures that appear on the light background using a dual-theme config (Option C).

However, re-evaluating against the actual problem scope:
