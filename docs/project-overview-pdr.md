...
| oxlint + oxfmt          | 1.57.0/0.42.0 | Linter and formatter (OXC toolchain)                |
...
### PDR-4: OXC toolchain over Biome

**Decision**: Use the OXC toolchain (oxlint for linting, oxfmt for formatting) instead of Biome.

**Rationale**:

- oxlint and oxfmt are written in Rust and provide faster execution than Biome, with more granular category-based rule configuration.
- oxlint's `correctness` and `suspicious` categories plus the `typescript`, `unicorn`, and `oxc` plugins cover the project's needs without a monolithic "recommended" preset.
- Configuration is split into `.oxlintrc.json` (linter) and `.oxfmtrc.json` (formatter). The project uses tabs for indentation and single quotes for strings.

**Consequence**: The `check` script runs oxlint (with `--fix`) then oxfmt in sequence. oxfmt handles import sorting and Tailwind class sorting natively.
