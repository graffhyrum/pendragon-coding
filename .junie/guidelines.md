...
- bun run lint — bunx oxlint --ignore-path .gitignore --fix . (autofixes where safe).
- bun run check — bunx oxlint --ignore-path .gitignore --fix . && bunx oxfmt . (combined checks with writes).
...
- Path alias: @assets/_ → src/assets/_.
...
OXC toolchain (oxlint for linting, oxfmt for formatting and import sorting)
...
- files.ignore includes public/\*_/_.\*; oxlint/oxfmt ignore public/ via ignorePatterns.
...
- Convention: Place tests anywhere named _.test.ts or _.test.js. A tests/ directory at the repo root is a reasonable default.
- Run: bun test
...
- bun run prebuild executes astro check. This performs project-wide diagnostics and is suitable as a fast pre-commit or pre-push guard alongside oxlint/oxfmt.
...
- Tabs for indentation, single quotes for JS/TS, organized imports enforced by oxfmt.
