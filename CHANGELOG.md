...
- cb485a0: Clear bun audit findings: raise transitive overrides and bump Astro to 7.2.1 (patched XSS) with esbuild >=0.28.1.
...
- 3c83e94: Release checkout uses optional `RELEASE_REPO_PAT` so automated tags can trigger deploy; add `workflow_dispatch` to deploy for manual tag deploys.
